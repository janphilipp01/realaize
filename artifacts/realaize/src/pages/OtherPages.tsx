...
import type { MarketLocation, MarketBenchmark } from '../models/types';
...
export function CashFlowPage() {
  const { assets } = useStore();
  const { t, lang } = useLanguage();
  const [scenario, setScenario] = useState<'base' | 'stress' | 'upside'>('base');
  const dateLocale = lang === 'de' ? 'de-DE' : 'en-GB';

  const scenarioMods = { base: 1, stress: 0.88, upside: 1.08 };
  const mod = scenarioMods[scenario];

  // Compute monthly income/expenses from NOI engine per asset
  ...
  const chartData = Array.from({ length: 18 }, (_, i) => {
    ...
    // Use NOI-based monthly values with small random variance for realistic chart
    const variance = 1 + (Math.sin(i * 2.7) * 0.04); // deterministic variance instead of random
    const inflow = totalMonthlyIncome * variance * (isForecast ? mod : 1);
  ...
  const totalAnnualInflow = totalMonthlyIncome * 12;
  const totalAnnualOutflow = totalMonthlyExpenses * 12;

  const scenarioLabels = {
    base: t('cashflow.baseCaseLabel'),
    stress: t('cashflow.stressLabel'),
    upside: t('cashflow.upsideLabel'),
  };
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <KPICard label="Avg. Monthly Income" value={formatEUR(totalAnnualInflow / 12, true)} status="good" />
        <KPICard label="Avg. Monthly Expenses" value={formatEUR(totalAnnualOutflow / 12, true)} status="neutral" />
        <KPICard label="Avg. Net CF/Month" value={formatEUR((totalAnnualInflow - totalAnnualOutflow) / 12, true)} status="good" />
        <KPICard label="Annual NOI (Avg.)" value={formatEUR((totalAnnualInflow - totalAnnualOutflow), true)} status="neutral" />
      </div>

      {/* Scenario toggle */}
      <div className="flex gap-2 mb-4">
        {(['base', 'stress', 'upside'] as const).map(s => (
          <button key={s} onClick={() => setScenario(s)}
            className="px-4 py-2 rounded-xl text-sm font-medium transition-all"
            style={{
              background: scenario === s ? (s === 'stress' ? 'rgba(248,113,113,0.15)' : s === 'upside' ? 'rgba(74,222,128,0.15)' : 'rgba(201,169,110,0.15)') : 'rgba(255,255,255,0.05)',
              color: scenario === s ? (s === 'stress' ? '#f87171' : s === 'upside' ? '#4ade80' : 'var(--accent)') : 'var(--text-secondary)',
              border: `1px solid ${scenario === s ? (s === 'stress' ? 'rgba(248,113,113,0.25)' : s === 'upside' ? 'rgba(74,222,128,0.25)' : 'rgba(201,169,110,0.25)') : 'rgba(255,255,255,0.08)'}`,
              cursor: 'pointer',
            }}>
            {s === 'base' ? t('cashflow.baseCase') : s === 'stress' ? t('cashflow.stress') : t('cashflow.upside')}
          </button>
        ))}
      </div>

      <GlassPanel style={{ padding: 24, marginBottom: 24 }}>
        <SectionHeader title={`${t('cashflow.forecast')} — ${scenarioLabels[scenario]}`} />
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={chartData}>
            ...
            <Tooltip contentStyle={{ background: 'rgba(255,255,255,0.97)', border: '1px solid rgba(0,0,0,0.10)', borderRadius: 10, fontSize: 12 }} formatter={(v: any) => [`${v}k EUR`]} />
            <Area type="monotone" dataKey="Income" stroke="#007aff" strokeWidth={2} fill="url(#cfGradIn)" />
            <Area type="monotone" dataKey="Net" stroke="#4ade80" strokeWidth={2} fill="url(#cfGradNet)" />
            <Area type="monotone" dataKey="Expenses" stroke="#f87171" strokeWidth={1.5} fill="none" strokeDasharray="4 4" />
          </AreaChart>
        </ResponsiveContainer>
      </GlassPanel>

      {/* Asset contribution */}
      <SectionHeader title={t('cashflow.contributionByAsset')} />
      <div className="grid grid-cols-3 gap-4">
        {assets.map(asset => {
          const cf = computeAssetMonthlyCashFlow(asset);
          const monthlyIn = Math.round(cf.monthlyIncome / 1000);
          const monthlyOut = Math.round(cf.monthlyExpenses / 1000);
          return (
            <GlassPanel key={asset.id} style={{ padding: 18 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#1c1c1e', marginBottom: 4 }}>{asset.name}</div>
              <div style={{ fontSize: 12, color: 'rgba(60,60,67,0.45)', marginBottom: 12 }}>{asset.city}</div>
              <div className="grid grid-cols-2 gap-2">
                <div><div style={{ fontSize: 10, color: 'rgba(60,60,67,0.45)' }}>{t('cashflow.income')}</div><div style={{ fontSize: 15, fontWeight: 700, color: '#4ade80' }}>{monthlyIn}k/Mo</div></div>
                <div><div style={{ fontSize: 10, color: 'rgba(60,60,67,0.45)' }}>{t('cashflow.expenses')}</div><div style={{ fontSize: 15, fontWeight: 700, color: '#f87171' }}>{monthlyOut}k/Mo</div></div>
              </div>
              <div className="progress-bar mt-3">
                <div className="progress-fill" style={{ width: `${(monthlyIn / (monthlyIn + monthlyOut)) * 100}%` }} />
              </div>
            </GlassPanel>
          );
        })}
